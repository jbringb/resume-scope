...
public class AnalysisTriggerIdempotencyRepository {
    ...
    public Mono<UUID> findAnalysisRunId(UUID jobRoleId, String idempotencyKey) {
        return Mono.from(dsl.select(ANALYSIS_RUN_ID)
                        .from(ANALYSIS_TRIGGER_IDEMPOTENCY)
                        .where(JOB_ROLE_ID.eq(jobRoleId).and(IDEMPOTENCY_KEY.eq(idempotencyKey))))
                .map(Record1::value1);
    }

    public Mono<Void> insert(UUID jobRoleId, String idempotencyKey, UUID analysisRunId) {
        return Mono.from(dsl.insertInto(ANALYSIS_TRIGGER_IDEMPOTENCY, JOB_ROLE_ID, IDEMPOTENCY_KEY, ANALYSIS_RUN_ID)
    ...
    public Mono<Void> repoint(UUID jobRoleId, String idempotencyKey, UUID analysisRunId) {
        return Mono.from(dsl.update(ANALYSIS_TRIGGER_IDEMPOTENCY)
